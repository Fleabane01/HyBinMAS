...
def sr_query_LLM(
        ...
        ret_info_str = None,
        with_dynamic_feedback = False,
        dynamic_feedback_list = None,
...
def sr_do_process_static(
        ...
        for callsite, callee_set in call_relation[offset].get('INTERNAL', {}).items(): # 没有则不处理
            for callee in callee_set:
                ...
                if key in callee_facts:
                    continue
                llm_success, callee_fact = sr_do_process_static(callee, cache, path, project_name, so_name, target_offset_map, all_funcs, call_relation, call_align_info, logger, timer)
                if callee_fact:
    ...
    timer.pause()
    response = sr_query_LLM(offset in target_offset_map, assembly_str, pseudo, callsite_to_callee_str, callee_facts, signature, args_info_str, ret_info_str, architecture=arch_info)
    timer.resume()
...
def sr_process_static(
    ...
    aligned_gt_path = os.path.join(PROJECTS_FOLDER, project_name, "aligned_ground_truth", convert_origin_to_dwarf_json(so_name))
    target_offset_map: Dict[str, str] = {}
    ...
    call_relation = {}
    with open(os.path.join(PROJECTS_FOLDER, project_name, "call_relation", f"{so_name}.json"), 'r', encoding='utf-8') as f:
        call_relation = json.load(f)
    with open(os.path.join(PROJECTS_FOLDER, project_name, "aligned_call", "aligned", f"{so_name}.json"), 'r', encoding='utf-8') as f:
        call_align_info = json.load(f)

    if use_call_relation:
        res_folder = os.path.join(PROJECTS_FOLDER, project_name, "res", "self")
    else:
        res_folder = os.path.join(PROJECTS_FOLDER, project_name, "res", "self_wo_call") # 没有使用call_relation的单独存放
    ...
    for target_offset in target_offset_map:
        ...
        try:
            timer = Timer()
            timer.start()
            llm_success, facts = sr_do_process_static(target_offset, cache, path, project_name, so_name, target_offset_map, all_funcs, call_relation, call_align_info, logger, timer, use_call_relation=use_call_relation, arch_info=arch_info)
            timer.stop()
            if facts:
                logger.info(f"[SR] FACTS, offset={target_offset}, func_name={facts['func_name']}, summarization={facts['summarization']}")
